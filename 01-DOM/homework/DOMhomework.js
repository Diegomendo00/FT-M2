...
let nombre = "Diego";
span.innerHTML = span.innerHTML + " " + nombre;
...
function buildToDo(todo, index) {
  // Tu código acá:
  let toDoShell = document.createElement("div"); //<div></div>
  toDoShell.setAttribute("class", "toDoShell"); //<div class = "toDoShell"></div>
  let toDoText = document.createElement("span"); //<span></span>
  toDoText.innerHTML = todo.description; //<span>"description"</span>
  toDoText.setAttribute("id", index); //<span id = index>"description</span>
  if (todo.complete) {
    toDoText.setAttribute("class", "completeText"); //<span id = index  class = "completeText">"description</span>+
  }
...
function addToDo() {
  // Tu código acá:
  let value = document.getElementById("toDoInput").value; //1
  let myToDo = new ToDo(value); //1
  toDoItems.push(myToDo); //2
  let inpt = document.getElementById("toDoInput"); //3
  inpt.value = ""; //3
  displayToDos();
}
